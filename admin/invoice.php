...
while ($row = $db->result())
{
	...
	$template->assign_block_vars('invoices', array(
			...
			'DATE' => $system->ArrangeDateAndTime($DATE),
			'INFO' => $info,
...
if ($PAGES > 1)
{
	...
	while ($COUNTER <= $PAGES && $COUNTER < ($PAGE + 6))
	{
		$template->assign_block_vars('pages', array(
				'PAGE' => ($PAGE == $COUNTER) ? '<b>' . $COUNTER . '</b>' : '<a href="' . $system->SETTINGS['siteurl'] . $system->SETTINGS['admin_folder'] . '/invoice.php?PAGE=' . $COUNTER . $pagenation_link . '"><u>' . $COUNTER . '</u></a>'
				));
		$COUNTER++;
	}
}
...
$template->assign_vars(array(
	...
	'PAGNATION' => ($PAGES > 1),
	'PREV' => ($PAGES > 1 && $PAGE > 1) ? '<a href="' . $system->SETTINGS['siteurl'] . $system->SETTINGS['admin_folder'] . '/invoice.php?PAGE=' . $PREV . $pagenation_link . '"><u>' . $MSG['5119'] . '</u></a>&nbsp;&nbsp;' : '',
	'NEXT' => ($PAGE < $PAGES) ? '<a href="' . $system->SETTINGS['siteurl'] . $system->SETTINGS['admin_folder'] . '/invoice.php?PAGE=' . $NEXT . $pagenation_link . '"><u>' . $MSG['5120'] . '</u></a>' : '',
	'PAGE' => $PAGE,
	'PAGES' => $PAGES,
