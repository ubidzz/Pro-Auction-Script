...
class pfcCommand_connect extends pfcCommand
{
  function run(&$xml_reponse, $p)
  {
    ...
    $nick = $params[0];
    $GlobalConfig = new pfcGlobalConfig();
    $UserConfig = new pfcUserConfig();
    $Container = new pfcContainer();
    
    $c  = $GlobalConfig->Instance();
    $u  = $UserConfig->Instance();
  ...
  function _resetChannelIdentifier($clientid)
  {
    ...
    {
    	$Command_join = new pfcCommand_join();    

      $chanrecip = $Command_join->GetRecipient($channame);
      $chanid    = $Command_join->GetRecipientId($channame);
