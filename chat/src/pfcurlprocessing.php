...
function pfc_make_hyperlink($text)
{
  ...
  $ret = preg_replace_callback("#(^|[\n ]])([\w]+?://[\w\#$%()&~/!.-;:=,?@+]*)#is", function($m) { return "$m[1]" . pfc_shorten_url($m[2]) . "";}, $ret);
  ...
  $ret = preg_replace_callback("#(^|[\n ]])((www|ftp).[\w\#$%&~/.-;:=,?@+]*)#is",function($m) { return "$m[1]" . pfc_shorten_url($m[2]) . "";}, $ret);

  // matches an email@domain type address at the start of a line, or after a space.
  // Note: Only the followed chars are valid; alphanums, "-", "_" and or ".".
  //$ret = preg_replace("#(^|[\n ])([a-z0-9&\-_.]+?)@([\w\-]+\.([\w\-\.]+\.)*[\w]+)#i", "\\1<a href=\"mailto:\\2@\\3\">\\2@\\3</a>", $ret);
  $ret = preg_replace_callback("#(^|[\n ]])([a-z0-9&-_.]+?)@([\w-]+.([\w-.]+.)*[\w]+)#i", function($m) { return "$m[1]" . pfc_shorten_url($m[2].'@'.$m[3]) . "";}, $ret);
